...
function reservationListAjax(page) {  // 위에서 보낸 매개변수 1을 받아줌
    ...
    const f = document.getElementById("form1"); //reservationList.html의 form1과 연결
    f.page.value = page;

    $.ajax({ // $부분이 jquery
        url: "/user/reservationListAjax", //백엔드 경로
        type: 'GET',
        cache: false,
        data: $('#form1').serialize(), // 이걸 통해 input 애들 값을 백엔드 경로로 보내줌 그러면 백엔드가 받는다.
        dataType: "html",
        async: false,
        //성공 시에 reservationList.html의 reservationListForm태그 위치에 백엔드 경로(reservationListAjax)에 연결 된 리턴 값인 프론트 reservationListAjax.html을 넣어서 보여줌
...
function paymentCancel(orderNo) {
    ...
    console.log(orderNo)
    $.ajax({
        url: "/user/paymentCancel/"+ orderNo, //백엔드 경로
        type: 'POST',
        cache: false,
        dataType: "html",
        async: false,
